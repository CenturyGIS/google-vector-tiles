...
const MVTLayer = require('./MVTLayer');
...
class MVTSource {
  ...
  _drawVectorTile(vectorTile, tileContext) {
    if (this._visibleLayers) {
      for (let i = 0, { length } = this._visibleLayers; i < length; i++) {
  ...
  _mouseEvent(event, callbackFunction, options) {
    ...
    for (let i = 0, { length } = clickableLayers; i < length; i++) {
  ...
  setSelectedFeatures(featuresIds) {
    ...
    this.deselectAllFeatures();
    for (let i = 0, { length } = featuresIds; i < length; i++) {
